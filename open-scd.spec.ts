...
describe('Custom plugins', () => {
  ...
  it('does not allow the loading of a plugin, already loaded', async () => {
    const currentEditorPluginCount = editor.loadedPlugins.menu.length;
    editor.plugins = {
      menu: [testMenuPlugin],
      editor: [],
    };
    await editor.updateComplete;
    expect(editor.loadedPlugins.menu.length).to.equal(currentEditorPluginCount);
  });

  it('does not allow the loading of a duplicate plugin', async () => {
    const { menu } = editor.plugins;
    const currentEditorPluginCount = editor.loadedPlugins.editor.length;
    editor.plugins = {
      menu: [...menu, ...menu],
      editor: [],
    };
    await editor.updateComplete;
    expect(editor.loadedPlugins.editor.length).to.equal(
      currentEditorPluginCount
    );
  });
});
